    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-text-secondary mt-0.5 font-medium">
              Registro de operaciones en la red {activeConfig.name}
            </p>
          </div>
        </div>
      ...
      <Card padding="none" className="overflow-hidden border border-border bg-bg-primary shadow-xs">
          ...
          <div className="flex flex-col items-center gap-3 py-14 px-6 text-center">
            ...
            <div>
              ...
              <p className="text-xs text-text-secondary mt-1">
                Tus transferencias en la red {activeConfig.name} aparecerán aquí.
              </p>
            </div>
          </div>
        )}
        ...
        {!isLoading && !error && transactions.length > 0 && (
          <div className="divide-y divide-border">
            {transactions.map((tx) => {
              const isSend = tx.tipo === 'send';
              const date = new Date(tx.fechaTransaccion);
              return (
                <div
                  key={tx.txHash}
                    ...
                    <div className="flex flex-col items-end">
                      <span
                        className={`font-bold text-sm font-mono ${
                          isSend ? 'text-accent-primary' : 'text-success'
                        }`}
                      >
                        {isSend ? '-' : '+'}{tx.monto} {activeConfig.symbol}
                      </span>
                      <span className="text-[10px] text-text-tertiary font-medium font-mono mt-0.5 capitalize">
                        {tx.estadoTransaccion} · {tx.red}
                      </span>
                    </div>

                    <a
                      href={`${activeConfig.explorerUrl}/tx/${tx.txHash}`}
                      target="_blank"
